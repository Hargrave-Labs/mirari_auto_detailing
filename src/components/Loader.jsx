...
const Loader = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        }, 4000); // Allow time for the path drawing
        return () => clearTimeout(timer);
    }, [onComplete]);

    // Precise paths from the provided SVG
    const svgPaths = [
    ...
    return (
        <AnimatePresence>
            {!finished && (
                <motion.div
                    className="fixed inset-0 z-50 flex items-center justify-center bg-mirari-black"
                // We remove the exit opacity fade because we want the logo to morph, not fade out.
                // The background 'bg-mirari-black' needs to disappear or be handled.
                // Actually, Hero has the same background.
                // So we can let the white background fade out, but keep the logo.
                // But here the background is part of the wrapper.
                // Let's make the wrapper fade out, but the logo stay?
                // No, framer motion layoutId needs the component to be present in the new tree.
                // So when Loader unmounts, Hero mounts with the same layoutId.
                >
                    <motion.div
                        layoutId="logo-container"
                        className="w-full max-w-2xl px-4 flex items-center justify-center"
                        // Smaller max-w as requested "tiniest bit smaller" - existing was max-w-4xl (which is huge 56rem), 2xl is 42rem.
                            ...
                            <g transform="translate(-68.000000,634.500000) scale(0.100000,-0.100000)" stroke="#FFFFFF" strokeWidth="15">
                                {svgPaths.map((d, index) => (
                                    <motion.path
                                        key={index}
                                        d={d}
                                        fill="#FFFFFF" // Initially transparent fill
                                        fillOpacity={0}
                                        initial={{ pathLength: 0, fillOpacity: 0 }}
                                        animate={{ pathLength: 1, fillOpacity: 1 }}
                                        transition={{
                                            pathLength: { duration: 2.5, ease: "easeInOut" },
                                            fillOpacity: { duration: 0.8, delay: 2.2 } // Fade in fill after drawing
                                        }}
                                    />
                                ))}
                            </g>
